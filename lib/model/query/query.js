...
class Query {
  constructor (query, Model, requelize, parse = true) {
    debug(`new query based on ${Model._name} : ${query.toString()} (parse: ${parse})`)
  ...
  run () {
    ...
    if (this._parse) {
      q = q.then((res) => {
        if (res) {
          return this._Model._parse(res, { setIsSaved: true })
        } else {
  ...
  feed () {
    ...
    this._query.changes().run((err, cursor_) => {
      ...
      cursor.each((err, data) => {
        ...
        debug(`feed has data ${JSON.stringify(data)}`)
  ...
  embed (tree) {
    ...
    Object.keys(tree).map((key) => {
      ...
      if (!Model._joins.hasOwnProperty(key)) {
        throw new RequelizeError(`Missing relationship ${Model._name}.${key}`)
      }

      if (!requelize.models.hasOwnProperty(Model._joins[key].model)) {
        throw new RequelizeError(`Missing model ${Model._joins[key].model} in relationship ${Model._name}.${key}`)
      }
