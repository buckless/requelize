const debug = require('debug')('requelize:ready')
...
const ensureTable = (model, { r, opts }) => {
  return ensureDb({ r, opts })
    .then(() => r.tableCreate(model._name, model._options).run())
    .catch((err) => {
      /* istanbul ignore else */
      if (err.message.match(/^Table `.*` already exists/)) {
        debug(`table ${model._name} exists`)
...
/**
 * Wrapper over ready function. Binder to requelize instance
 * @param  {Requelize} requelize requelize instance
